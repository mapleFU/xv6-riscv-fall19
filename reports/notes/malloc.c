...
// using sbrk to allocate more heap.
static Header *moreheap(size_t nu)
{
    char *cp;
    Header *up;

    // sbrk is expensive, so maybe allocate memory for a lot.
    if (nu < NALLOC)
        nu = NALLOC;
...
kr_malloc(size_t nbytes)
{
    ...
    if ((prevp = freep) == NULL) {	/* no free list yet */
        // if no, start with base.
        // first initialize the list to a self pointed ring, and make it size 0.
...
{
    Header *bp, *p;
    // well, free NULL is safe...?
    if (ap == NULL)
        return;
    bp = (Header *) ap - 1;	/* point to block header */
    // return it to a fiting place.
    // If it's in the end, just put it to the end.
...
// 8 blocks).
//
// This data structure is for a size.
struct sz_info {
    struct bd_list free; // The freelist in a size
    char *alloc;    // an array alloc to keep track which blocks have been allocated
    char *split;    // an split array to keep track which has been split.
};
